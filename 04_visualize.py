"""
04_visualize.py
Campus Placement Analytics - Step 4: Publication-Quality Data Visualizations

This script reads placement_clean.csv and generates 6 high-resolution PNG charts
using Matplotlib and Seaborn, saving them into the charts/ directory.
"""
...
def create_visualizations():
    ...
    print("=" * 60)
    print("STEP 4: GENERATING DATA VISUALIZATIONS")
    print("=" * 60)
    print(f"Reading cleaned data from '{csv_path}'...")
    df = pd.read_csv(csv_path)
    
    palette_primary = ["#1f77b4", "#ff7f0e"]
    status_palette = {"Placed": "#2ca02c", "Not Placed": "#d62728"}
    
    # -------------------------------------------------------------------------
    # Chart 1: Placement Rate by Specialisation
    # -------------------------------------------------------------------------
    print("Generating Chart 1: Placement Rate by Specialisation...")
    ...
    plt.title('Placement Rate by MBA Specialisation', fontsize=14, fontweight='bold', pad=15)
    plt.xlabel('MBA Specialisation', fontsize=11, fontweight='bold')
    ...
    plt.tight_layout()
    chart1_path = os.path.join(charts_dir, 'placement_rate_by_specialisation.png')
    ...
    plt.title('Placement Rate by Prior Work Experience', fontsize=14, fontweight='bold', pad=15)
    plt.xlabel('Work Experience (Yes / No)', fontsize=11, fontweight='bold')
    plt.ylabel('Placement Rate (%)', fontsize=11, fontweight='bold')
    plt.ylim(0, 100)
    ...
    # -------------------------------------------------------------------------
    print("Generating Chart 3: Salary Distribution for Placed Students...")
    placed_df = df[df['status'] == 'Placed']
    plt.figure(figsize=(9, 5.5))
    
    ax = sns.histplot(placed_df['salary'], kde=True, color='#2b5c8f', bins=15)
    ...
    plt.title('Salary Offer Distribution (Placed Students)', fontsize=14, fontweight='bold', pad=15)
    plt.xlabel('Salary (INR)', fontsize=11, fontweight='bold')
    ...
    # -------------------------------------------------------------------------
    # Chart 4: Average Salary by Specialisation and Gender
    # -------------------------------------------------------------------------
    print("Generating Chart 4: Average Salary by Specialisation & Gender...")
    plt.figure(figsize=(8, 5.5))
    sal_spec_gender = placed_df.groupby(['specialisation', 'gender'])['salary'].mean().reset_index()
    
    ax = sns.barplot(
        data=sal_spec_gender, 
        x='specialisation', 
        y='salary', 
        hue='gender', 
        palette=['#3498db', '#e74c3c']
    )
    plt.title('Average Salary Offer by Specialisation & Gender', fontsize=14, fontweight='bold', pad=15)
    plt.xlabel('MBA Specialisation', fontsize=11, fontweight='bold')
    ...
    # -------------------------------------------------------------------------
    # Chart 5: Key Chart - Academic Score Averages Comparison (Placed vs Unplaced)
    # -------------------------------------------------------------------------
    print("Generating Chart 5: Key Academic Comparison (Placed vs Not Placed)...")
    plt.figure(figsize=(10, 6))
    
    academic_cols = {'ssc_p': '10th (SSC)', 'hsc_p': '12th (HSC)', 'degree_p': 'Degree', 'mba_p': 'MBA'}
    ...
    # -------------------------------------------------------------------------
    # Chart 6: Correlation Heatmap of Numeric Academic Scores
    # -------------------------------------------------------------------------
    ...
    labels = ['10th (SSC)', '12th (HSC)', 'Degree %', 'E-Test %', 'MBA %', 'Placed Flag']
    ...
    plt.title('Correlation Matrix: Academic Scores & Placement Status', fontsize=14, fontweight='bold', pad=15)
    ...
    print("\nVisualization generation step completed successfully!")
    print("=" * 60)
